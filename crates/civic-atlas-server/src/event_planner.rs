...
use civic_atlas_types::event_planner::{
    ...
    EventLayerListRequest, EventLayerListResponse, EventPlannerService,
    IntakePendingVendorRequest, IntakePendingVendorResponse, Placement, PlacementCreateRequest,
    PlacementDeleteRequest, PlacementListRequest, PlacementListResponse,
    ...
    TaskDeleteRequest, TaskListRequest, TaskListResponse, TaskMutationResponse,
    TaskUpdateRequest,
};
...
fn note_from_row(row: &PgRow) -> PlacementNote {
    PlacementNote {
        ...
        author_display: row.try_get::<String, _>("author_display").unwrap_or_default(),
...
fn ts_ms(row: &PgRow, column: &str) -> i64 {
    if let Ok(Some(ts)) = row.try_get::<Option<OffsetDateTime>, _>(column) {
        return ts.unix_timestamp() * 1_000
            + i64::from(ts.millisecond());
    }
...
async fn resolve_event_layer_id(
    ...
    let row = sqlx::query(
        r#"SELECT id FROM event_layers WHERE tenant_id = $1 AND slug = $2"#,
    )
    .bind(tenant_id)
    .bind(slug)
    .fetch_optional(&mut **tx)
